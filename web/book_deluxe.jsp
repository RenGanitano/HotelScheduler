<%-- 
    Document   : book_deluxe
    Created on : 2-Apr-2014, 11:58:19 PM
    Author     : Renniel
--%>
...
<%
    if(session.getAttribute("Session") == "Guest") {
        ...
        dbh.connect();
        if(dbh.select("SELECT * FROM HOTELROOMS WHERE BOOKED = 'N' AND ROOMTYPE = 'Deluxe'", "ROOM#").size() == 0 && dbh.select("SELECT * FROM HOTELROOMS WHERE BOOKED = 'N' AND ROOMTYPE = 'Deluxe with view'", "ROOM#").size() == 0) {
        response.sendRedirect("service.jsp");
    } else {
...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-lg-12">
                <h1 class="page-header">Deluxe
                    <small>Room</small>
                </h1>
                <ol class="breadcrumb">
                    <li><a href="index.jsp">Home</a>
                    </li>
                    <li class="active">Deluxe</li>
                </ol>
            </div>
        ...
        <div class="row">
          ...
          <form name="bookingform" action="">
            ...
            <br /> <br />
            Would you like a view?(Check for Yes): 
            <input type="checkbox" name="viewYes" value="Yes" />
            <br /> <br />
            ...
            <br /> <br />
            <input type="submit" name='submitDeluxe' value="Submit">
            <input type="hidden" name='previousurl' value="book_deluxe.jsp">
...
<%
    if(request.getParameter("submitDeluxe") != null) {        
        if(request.getParameter("viewYes") != null) {
            dbh.connect();
            customer = new Customer(((Customer)session.getAttribute("Session")), "Deluxe with view");
            try {
                i = Integer.parseInt(dbh.select("SELECT * FROM HOTELROOMS WHERE ROOMTYPE = 'Deluxe with view' AND BOOKED = 'N'", "ROOM#").get(0).toString());                            
            ...
            dbh.connect();
            customer = new Customer(((Customer)session.getAttribute("Session")), "Deluxe");
            try {
                i = Integer.parseInt(dbh.select("SELECT * FROM HOTELROOMS WHERE ROOMTYPE = 'Deluxe' AND BOOKED = 'N'", "ROOM#").get(0).toString());                            
                customer.getRoom().roomNum = String.valueOf(i);
                dbh.insertBookRoom(i, customer.getUsername(), request.getParameter("specialRequest"), request.getParameter("startDate"), request.getParameter("finishDate"));            
                dbh.insert("UPDATE HOTELROOMS SET BOOKED = 'Y' WHERE ROOM# = " + customer.getRoom().roomNum);
            } catch(NullPointerException e) {
	...
	request.setAttribute("previousurl", "book_deluxe.jsp");
